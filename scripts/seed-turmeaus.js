...
async function seedTurmeaus() {
  const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'turmeaus-data.json'), 'utf8'));
  
  // Filter out accessories (humidors, cases, cutters, etc.)
  const cigarsOnly = data.filter(p => {
    const lower = p.name.toLowerCase();
    if (lower.includes('humidor') || lower.includes('cigar case') || lower.includes('cutter') ||
        lower.includes('ashtray') || lower.includes('lighter') || lower.includes('punch') ||
        lower.includes('travel case') || lower.includes('finger case') || lower.includes('cabinet') ||
        lower.includes('drawer') || lower.includes('leather case')) return false;
    return true;
  });
  
  console.log(`🏪 Turmeaus: ${data.length} products, ${cigarsOnly.length} cigars to process\n`);

  // Clean previous Turmeaus data
  const oldPrices = await sql`SELECT COUNT(*) as c FROM cs_prices WHERE retailer = 'Turmeaus'`;
  if (parseInt(oldPrices[0].c) > 0) {
    console.log(`🗑️  Removing ${oldPrices[0].c} existing Turmeaus prices...`);
  ...
  for (let i = 0; i < cigarsOnly.length; i++) {
    const p = cigarsOnly[i];
    ...
    if (bestScore >= 0.7 && bestMatch) {
      ...
      matched++;
      if (matched <= 5) {
        console.log(`  ✅ MATCH (${(bestScore * 100).toFixed(0)}%): "${p.name.substring(0, 50)}" → "${bestMatch.name.substring(0, 40)}"`);
      }
    } else {
      ...
      if (newProducts <= 5) {
        console.log(`  🆕 NEW: "${p.name.substring(0, 55)}" (best was ${(bestScore * 100).toFixed(0)}%)`);
      }
    }

    // Insert price with error handling
    try {
      await sql`
        INSERT INTO cs_prices (
          ...
          ${productId}, ${'Turmeaus'}, ${'https://www.turmeaus.co.uk'},
          ${p.price}, ${'GBP'},
          ${true}, ${p.url}, ${p.name}, ${'turmeaus-' + i},
          ${new Date()}
        )
      `;
    } catch (e) {
      console.log(`  ⚠️ Price insert failed for ${p.name}: ${e.message}`);
      skipped++;
      continue;
    }

    if ((matched + newProducts) % 500 === 0) {
      console.log(`  ... processed ${matched + newProducts + skipped}/${cigarsOnly.length}`);
    }
  }
