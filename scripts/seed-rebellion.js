...
function normalise(name) {
  return name
    ...
    // Rebellion-specific: remove brand prefix before colon
    .replace(/^[^:]+:\s*/i, '')
    .replace(/[^a-z0-9 ]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
...
async function seedRebellion() {
  const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'rebellion-data.json'), 'utf8'));
  console.log(`🏴 Rebellion Cigars: ${data.length} products to process\n`);

  // Clean previous Rebellion data
  const oldPrices = await sql`SELECT COUNT(*) as c FROM cs_prices WHERE retailer = 'Rebellion'`;
  if (parseInt(oldPrices[0].c) > 0) {
    console.log(`🗑️  Removing ${oldPrices[0].c} existing Rebellion prices...`);
    await sql`DELETE FROM cs_prices WHERE retailer = 'Rebellion'`;
  }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    await sql`
      INSERT INTO cs_prices (
        ...
        ${productId}, ${'Rebellion'}, ${'https://www.rebellioncigars.co.uk'},
        ${p.price}, ${'GBP'},
        ${p.available !== false}, ${p.url}, ${p.name}, ${'rebellion-' + i},
        ${new Date()}
      )
    `;

    if ((matched + newProducts) % 50 === 0) {
