 ...
 * Adapted for cs_products schema.
 */
...
const categoryUrls = [
  ...
  'new-world-cigars-juliany-cigars-c-317_102_2967.html',
  'new-world-cigars-luis-martinez-cigars-c-317_102_122.html',
  'new-world-cigars-la-galera-cigars-c-317_102_2956.html',
  'new-world-cigars-estrella-cigars-c-317_102_2958.html',
  'new-world-cigars-meerapfel-cigars-c-317_102_3181.html',
  'new-world-cigars-silencio-cigars-c-317_102_3182.html',
  'new-world-cigars-chateau-diadem-cigars-c-317_102_3107.html',
  'new-world-cigars-diamond-crown-cigars-c-317_102_3022.html',
  'new-world-cigars-cle-cigars-c-317_102_2954.html',
  'cigar-accessories-cigar-samplers-c-317_103_249.html',
];
...
// Build multiple match keys for fuzzy matching
function matchKeys(name, brand) {
  const keys = new Set();
  ...
  if (brand) {
    ...
    if (full.startsWith(b)) {
      keys.add(full.slice(b.length).trim());
    }
  }
  return keys;
}
...
async function scrape() {
  ...
  const products = await sql`
    SELECT id, name, brand FROM cs_products WHERE image_url IS NULL
  `;
  
  // Build lookup: normalised name -> product id
  const lookup = new Map();
  for (const p of products) {
    for (const key of matchKeys(p.name, p.brand)) {
      if (key.length < 3) continue;
      if (!lookup.has(key)) lookup.set(key, []);
      lookup.get(key).push(p.id);
    }
  }
  ...
  if (title.includes('Just a moment')) {
    console.log('❌ Cloudflare blocked. Try headless: false');
    await browser.close();
    return;
  }
  ...
  for (const catPath of categoryUrls) {
    ...
    const brandLabel = catPath.split('-c-')[0].replace(/.*cigars-/, '').replace(/-/g, ' ');
    ...
    try {
      ...
      while (currentUrl && pageNum <= 30) {
        ...
        const pageProducts = await page.evaluate(() => {
          ...
          // Method 1: product links with images
          ...
          for (const link of links) {
            ...
            const src = img.src;
            if (src.includes('placeholder') || src.includes('logo') || src.includes('icon') || src.includes('banner')) continue;
            ...
            let imageUrl = src;
            // CGars often has thumbnails in category, replace with full size
            imageUrl = imageUrl.replace(/\/thumbnails\//, '/');
            
            results.push({ name, imageUrl });
          }
          
          // Method 2: product grid items (fallback)
          if (results.length === 0) {
            const imgs = document.querySelectorAll('img[alt]');
            for (const img of imgs) {
              if (!img.src || (img.naturalWidth > 0 && img.naturalWidth < 50)) continue;
              if (img.src.includes('logo') || img.src.includes('icon') || img.src.includes('banner') || img.src.includes('404')) continue;
              if (img.alt && img.alt.length > 10 && !seen.has(img.alt)) {
                seen.add(img.alt);
                results.push({ name: img.alt, imageUrl: img.src.replace(/\/thumbnails\//, '/') });
              }
            }
          }
        ...
        const nextUrl = await page.evaluate(() => {
          ...
          for (const a of links) {
            ...
            if (text === '»' || text === 'Next' || text === 'next' || text.includes('Next »')) {
              return a.href;
            }
          }
          return null;
      ...
      for (const prod of allProducts) {
        ...
        if (!ids) {
          const shorter = normName.replace(/\s*(tubed|untubed|tubos)$/i, '').trim();
        ...
        if (!ids) {
          ...
          for (const [key, keyIds] of lookup) {
            if (key.length > 5 && normName.includes(key) && key.length > bestLen) {
              bestMatch = keyIds;
              bestLen = key.length;
            }
          }
          if (bestLen > 15) ids = bestMatch;
        }
      ...
      if (allProducts.length > 0) {
        console.log(`  ${brandLabel}: ${allProducts.length} found, ${matched} matched${pageNum > 1 ? ` (${pageNum} pages)` : ''}`);
      }

      await page.waitForTimeout(300 + Math.random() * 500);
    } catch (e) {
      console.log(`  ⚠️ ${brandLabel}: ${e.message.slice(0, 60)}`);
    }
  }
  ...
  console.log(`\n✅ Updated ${totalUpdated} product images`);
  console.log(`📸 Still missing: ${remaining[0].c}`);
  
  const total = await sql`
    SELECT COUNT(*) as total, 
      COUNT(image_url) FILTER (WHERE image_url IS NOT NULL) as with_img
    FROM cs_products
  `;
